...
1. 点击右侧调色盘的“从原图取色”，在弹窗中使用生成原图、参考图，或单独选择一张取色图片。也可以粘贴或拖入图片。
2. 点击图片中的目标位置，查看参照色及其 HEX / RGB。可放大并滚动图片取细节，也可聚焦图片后用方向键逐像素取色。
3. 弹窗列出当前色卡中最接近的 6 个拼豆色号，点击即可设为当前颜色并加入最近使用。之后可继续画笔、填充或换色操作；选色本身不会修改图纸。

透明区域无法取色；半透明像素按白底显示色匹配。屏幕色和实物可能有差异，可对照参照色自行选用候选。关闭弹窗或按 Esc 可取消。重新打开项目后若原图未加载，可在弹窗中重新选择取色图片。

局部批量换色：

1. 点击工具栏“换色”，在右侧选择“框选区域”。
2. 在主画布拖出矩形选区，反向拖动也可以；重新拖框可调整范围，“清除选区”可取消。
3. 在“原颜色”中选择要替换的色号，再从调色盘选择新颜色。右侧会显示选区大小和将替换的拼豆数量。
4. 点击“替换选区内同色”。只处理框内所有可见、未锁定图层的同色拼豆，框外、其它颜色和空格保持原样。整次操作可一次撤销或重做。

选择“全部图纸”可使用原有换色方式：先在调色盘选新颜色，再点击画布中的原颜色。两种范围均遵循“仅显示当前图层”的显示范围；切换工具、图层或画布尺寸后需要重新框选。
