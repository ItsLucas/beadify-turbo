...
(async () => {
  ...
  try {
    ...
    checks.push('All model/upload endpoints unavailable; traversal rejected');
    ...
    assert.match(refused, /关闭 OCR 和 VLM/); checks.push('AI UI hidden and client model calls rejected before network access');
    ...
    checks.push('CPU optimized generation, hard color budget and manual text annotation; no AI controls after upload');
    ...
    checks.push('Zero model/API requests, zero external requests, zero page errors');
